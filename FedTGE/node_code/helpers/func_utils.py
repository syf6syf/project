...
def get_split(args,data, device):
    ...
    perm = rs.permutation(data.num_nodes) # 对 num_nodes 个节点进行随机排列，得到一个打乱的索引列表 perm。
    train_number = int(args.ratio_training*len(perm))
    idx_train = torch.tensor(sorted(perm[:train_number])).to(device) #  取出 train_number 个随机节点作为训练集索引
