...
def split_Random(args, data):
    """
    Splits the graph nodes randomly across clients.
    """
    ...
    sampling_rate = (np.ones(client_num) - ovlap) / client_num
    data.index_orig = torch.arange(data.num_nodes)
    ...
    print("Setting node attributes")
    nx.set_node_attributes(G,
        dict([(nid, nid) for nid in range(nx.number_of_nodes(G))]),
        name="index_orig"
    )

    # ✅ 修复点1：初始化 indices（打乱节点顺序）
    indices = torch.randperm(data.num_nodes)

    # ✅ 修复点2：初始化 client_node_idx
    client_node_idx = {idx: [] for idx in range(client_num)}

    # 分配主节点
    sum_rate = 0
    for idx, rate in enumerate(sampling_rate):
        client_node_idx[idx] = indices[round(sum_rate * data.num_nodes): round((sum_rate + rate) * data.num_nodes)]
        sum_rate += rate
    ...
    if ovlap:
        ovlap_nodes = indices[round(sum_rate * data.num_nodes):]
        for idx in client_node_idx:
            client_node_idx[idx] = torch.cat((client_node_idx[idx], ovlap_nodes))

    # Drop edge (optional)
    if drop_edge:
        ovlap_graph = nx.Graph(nx.subgraph(G, ovlap_nodes.tolist()))
        ovlap_edge_ind = np.random.permutation(ovlap_graph.number_of_edges())
        drop_all = ovlap_edge_ind[:round(ovlap_graph.number_of_edges() * drop_edge)]
        drop_client = [
            drop_all[s:s + round(len(drop_all) / client_num)]
            for s in range(0, len(drop_all), round(len(drop_all) / client_num))
        ]
    ...
    for owner in client_node_idx:
        nodes = client_node_idx[owner]
        sub_g = nx.Graph(nx.subgraph(G, nodes.tolist()))
        if drop_edge:
            sub_g.remove_edges_from(np.array(ovlap_graph.edges)[drop_client[owner]])
        graphs.append(from_networkx(sub_g))
